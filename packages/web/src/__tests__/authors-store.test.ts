...
vi.mock('@loikmon/api', () => ({
  authors: {
    fetchAuthors:   (...a: any[]) => mockFetchAuthors(...a),
    getAuthor:      (...a: any[]) => mockGetAuthor(...a),
    followUnfollow: (...a: any[]) => mockFollowUnfollow(...a),
  }
}))

import { useAuthorsStore } from '../stores/authors'

const makeAuthor = (id: number) => ({ id, name: `Author ${id}`, thumbnail: '', bookscount: id, articlescount: 0 })
...
describe('authors store', () => {
  ...
  describe('fetchAuthors()', () => {
    ...
    it('replaces list on page 0 (first load)', async () => {
      ...
      const store = useAuthorsStore()
      store.list = [makeAuthor(99)] as any
  ...
  describe('toggleFollow()', () => {
    it('updates detail.is_following on match', async () => {
      ...
      const store = useAuthorsStore()
      store.detail = { ...makeAuthor(5), is_following: false } as any
