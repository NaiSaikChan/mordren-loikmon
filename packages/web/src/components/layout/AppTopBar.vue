...
<template>
  <header class="h-14 flex items-center gap-3 px-4 bg-white dark:bg-surface-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-10">
    ...
    <button
      ...
      :aria-label="t('common.more') || 'Toggle navigation menu'"
    ...
    <form class="flex-1 max-w-xl" role="search" @submit.prevent="handleSearch">
      <div class="relative">
        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm" aria-hidden="true">🔍</span>
        <input
          v-model="searchQuery"
          type="search"
    ...
    <div class="flex items-center gap-1 ml-auto">
      ...
      <button
        ...
        :aria-label="`Switch language to ${uiStore.locale === 'en' ? 'Mon' : 'English'}`"
        @click="toggleLocale"
      >
      ...
      <button
        ...
        :aria-label="`Change theme. Currently: ${uiStore.theme}`"
        @click="cycleTheme"
      >
