...
if (__VLS_ctx.catStore.list.length) {
    ...
    __VLS_asFunctionalElement(__VLS_intrinsicElements.button, __VLS_intrinsicElements.button)({
        ...
        ...{ class: (['shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors',
                !__VLS_ctx.selectedCat ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-surface-800 text-gray-600 dark:text-gray-300']) },
    });
    for (const [cat] of __VLS_getVForSourceType((__VLS_ctx.catStore.list))) {
        __VLS_asFunctionalElement(__VLS_intrinsicElements.button, __VLS_intrinsicElements.button)({
            ...
            key: (cat.id),
            ...{ class: (['shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors',
                    __VLS_ctx.selectedCat === String(cat.id) ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-surface-800 text-gray-600 dark:text-gray-300']) },
        });
        (cat.name);
    }
}
